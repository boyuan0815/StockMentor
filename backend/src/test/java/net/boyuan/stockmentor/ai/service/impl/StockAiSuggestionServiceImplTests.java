...
class StockAiSuggestionServiceImplTests {
        ...
        void setUp() {
                ...
                lenient().when(watchlistRepository.findByUserUserIdAndSymbolIn(eq(1L), anyCollection()))
                                .thenReturn(List.of());
                lenient().when(snapshotRepository.findTopBySymbolAndTimeframeOrderByCreatedAtDesc(anyString(),
                ...
                lenient().when(batchRepository
                                .findTopByUserUserIdAndModelAndPromptVersionAndInputHashOrderByCreatedAtDesc(
                                                ...
                                                anyString()))
                                .thenReturn(Optional.empty());
                lenient().when(behaviorProfileService.getBehaviorSummaryForSuggestion(1L))
        ...
        void reusableSameInputBatchRefreshesExpiredItemsInsteadOfReturningEmptyResponse() {
                ...
                when(itemRepository.findBySuggestionBatchAndStatusInOrderByRankNoAsc(eq(existingBatch),
                                ...
                                .thenAnswer(invocation -> {
                                        ...
                                        List<StockAiSuggestionItemStatus> statuses = new ArrayList<>(
                                                        (java.util.Collection<StockAiSuggestionItemStatus>) invocation
                                                                        .getArgument(1));
                ...
                when(itemRepository.save(any(StockAiSuggestionItem.class)))
                ...
                assertEquals(List.of("KO", "JNJ", "MSFT"),
                                response.suggestedStocks().stream().map(stock -> stock.symbol()).toList());
                assertTrue(existingBatch.getExpiresAt().isAfter(oldExpiresAt));
                assertTrue(expiredItems.stream()
                                .allMatch(item -> item.getStatus() == StockAiSuggestionItemStatus.ACTIVE));
        ...
        void reusableSameInputBatchExpiresDuplicateVisibleRankAndKeepsNewestActiveRow() {
                ...
                List<StockAiSuggestionItem> batchItems = new ArrayList<>(
                                List.of(staleNvda, latestKo, expiredJnj, expiredMsft));
        ...
        void refreshStoresRuleBasedFallbackWhenOpenAiFailsAndNoCacheExists() {
                ...
                assertEquals(
                                ...
                                response.suggestedStocks().stream().map(stock -> stock.matchScore()).distinct()
                                                .count());
                ...
                verify(batchRepository).save(
                                argThat(batch -> batch.getStatus() == StockAiSuggestionBatchStatus.FALLBACK_RULE_BASED
        ...
        void openAiSuccessUpdatesExistingSameInputFallbackCachedBatchWithoutDuplicateInsert() {
                ...
                StockAiSuggestionItem staleAggressiveItem = suggestionItem(existingFallbackCached,
                ...
                when(itemRepository.findBySuggestionBatchOrderByRankNoAsc(existingFallbackCached))
                                .thenReturn(batchItems);
                when(batchRepository.save(any(StockAiSuggestionBatch.class)))
                                .thenAnswer(invocation -> invocation.getArgument(0));
                when(itemRepository.save(any(StockAiSuggestionItem.class))).thenAnswer(invocation -> {
                        StockAiSuggestionItem item = invocation.getArgument(0);
                ...
                when(itemRepository.findBySuggestionBatchAndStatusInOrderByRankNoAsc(eq(existingFallbackCached),
                                anyCollection()))
                                .thenAnswer(invocation -> {
                                        @SuppressWarnings("unchecked")
                                        List<StockAiSuggestionItemStatus> statuses = new ArrayList<>(
        ...
        void rejectsSteadyMovementWhenPriceConsistencyIsChoppy() {
                ...
                assertTrue(retryPrompt.path("previousValidationError").asText()
                                .contains("AI wording contradicts volatile or choppy snapshot data"));
                assertTrue(retryPrompt.path("retryConstraints").toString()
                                .contains("Do not use steady, smooth, consistent, stable, or clear trend wording"));
                verify(batchRepository).save(
                                argThat(batch -> batch.getStatus() == StockAiSuggestionBatchStatus.FALLBACK_RULE_BASED
        ...
        void retryPromptUsesNaturalFactorGuidanceWhenAiOmitsInputFactors() {
                ...
                JsonNode retryPrompt = readJson(promptCaptor.getAllValues().get(1));
                assertTrue(retryPrompt.path("previousValidationError").asText()
                                .contains("AI detail reason does not mention enough input factors"));
        ...
        void repeatedTrendLabelWordingIsSanitizedWithoutRetry() {
                ...
                verify(batchRepository).save(argThat(batch -> batch.getStatus() == StockAiSuggestionBatchStatus.SUCCESS
                                && batch.getErrorMessage() == null));
                verify(itemRepository).save(argThat(item -> item.getDetailReason().contains("strong uptrend")
        ...
        void rejectsClearTrendWhenSnapshotIsVolatileAndChoppy() {
                ...
                verify(openAiClient, times(2)).generateSuggestion(anyString(), anyString());
                assertTrue(promptCaptor.getAllValues().get(1)
                                .contains("AI wording says clear trend without supporting snapshot data"));
                verify(batchRepository).save(
                                argThat(batch -> batch.getStatus() == StockAiSuggestionBatchStatus.FALLBACK_RULE_BASED
                                                && "retry unavailable".equals(batch.getErrorMessage())));
        }
        ...
        void rejectsStableMovementButAllowsStableVolumeWhenSnapshotIsVolatileAndChoppy() {
                ...
                verify(openAiClient, times(2)).generateSuggestion(anyString(), anyString());
                assertTrue(promptCaptor.getAllValues().get(1)
                                .contains("AI stable wording contradicts volatile or choppy snapshot data"));
                verify(batchRepository).save(argThat(batch -> batch.getStatus() == StockAiSuggestionBatchStatus.SUCCESS
                                && batch.getErrorMessage() == null));
        }
        ...
        void bannedAdviceWordingInSuggestionLabelIsRejected() {
                ...
                verify(openAiClient, times(2)).generateSuggestion(anyString(), anyString());
                assertTrue(promptCaptor.getAllValues().get(1)
                                .contains("AI returned advice, prediction, or unsupported external reason"));
                verify(batchRepository).save(
                                argThat(batch -> batch.getStatus() == StockAiSuggestionBatchStatus.FALLBACK_RULE_BASED
                                                && "retry unavailable".equals(batch.getErrorMessage())));
        }
        ...
        void existingFallbackCachedSameInputHashCreatesNewSuccessBatchAndPreservesFallbackHistory() {
                ...
                fallbackCached.setPromptVersion("stock-suggestion-v4");
                ...
                StockAiSuggestionItem fallbackCachedMsft = suggestionItem(fallbackCached,
                                StockAiSuggestionItemStatus.ACTIVE);
                fallbackCachedMsft.setSuggestionItemId(91L);
                fallbackCachedMsft.setAnalysisSnapshot(snapshot("MSFT"));
                StockAiSuggestionItem fallbackCachedAapl = suggestionItem(fallbackCached,
                                StockAiSuggestionItemStatus.ACTIVE);
                ...
                List<StockAiSuggestionItem> batchItems = new ArrayList<>(
                                List.of(fallbackCachedMsft, fallbackCachedAapl));
                ...
                assertEquals("Microsoft is a profile-aligned paper-trading learning suggestion based on structured data.",
                                savedSuccessBatch.get().getBatchSummary());
                ...
                assertEquals("Microsoft has risk category moderate, volatility moderate, trend is strong uptrend, price consistency smooth upward movement, volume trend stable, and data quality complete for beginner-friendly paper-trading practice.",
                                savedMsft.getDetailReason());
                assertEquals(snapshot("MSFT").getAnalysisSnapshotId(),
                                savedMsft.getAnalysisSnapshot().getAnalysisSnapshotId());
        ...
        void lowBehaviorWeightAndPromptSectionsAreIncluded() {
                ...
                when(batchRepository.save(any(StockAiSuggestionBatch.class))).thenAnswer(invocation -> {
                        StockAiSuggestionBatch batch = invocation.getArgument(0);
                        batch.setSuggestionBatchId(
                                        batch.getSuggestionBatchId() == null ? 20L : batch.getSuggestionBatchId());
                        return batch;
                });
                when(itemRepository.findBySuggestionBatchAndStatusInOrderByRankNoAsc(any(), anyCollection()))
                                .thenReturn(List.of());
                ...
                verify(openAiClient, atLeastOnce()).generateSuggestion(anyString(),
                                contains("\"declaredOnboardingProfile\""));
                verify(openAiClient, atLeastOnce()).generateSuggestion(anyString(),
                                contains("\"observedPaperTradingBehavior\""));
                verify(openAiClient, atLeastOnce()).generateSuggestion(anyString(),
                                contains("\"personalizationWeight\":{\"onboardingWeight\":80,\"behaviorWeight\":20"));
                verify(openAiClient, atLeastOnce()).generateSuggestion(anyString(),
                                contains("\"candidateFitSignals\""));
        }
        ...
        void behaviorSummaryUpdatedAtDoesNotChangeInputHash() {
                UserInvestmentProfile profile = profile();
                String firstHash = inputHashForGeneratedFallback(profile,
                                behaviorSummary(LocalDateTime.of(2026, 6, 8, 9, 0)));
                String secondHash = inputHashForGeneratedFallback(profile,
                                behaviorSummary(LocalDateTime.of(2026, 6, 8, 10, 0)));
        ...
        void meaningfulBehaviorFieldsChangeInputHash() {
                UserInvestmentProfile profile = profile();
                String baseline = inputHashForGeneratedFallback(profile,
                                behaviorSummary(LocalDateTime.of(2026, 6, 8, 9, 0)));
        ...
        void promptIncludesSortedCandidateFitSignalsWithConservativeGuardrail() throws Exception {
                ...
                assertTrue(root.has("currentPortfolioExposure"));
                ...
                assertFalse(root.path("currentPortfolioExposure").path("hasOpenPositions").asBoolean());
                ...
                for (JsonNode signal : candidateSignals) {
                        ...
                        assertTrue(signal.has("portfolioCompatibility"));
        ...
        @Test
        void promptIncludesConcentratedConservativePortfolioExposureInRanking() throws Exception {
                ...
                BehaviorSummaryForSuggestion aggressiveBehavior = behaviorSummary(
                ...
                when(behaviorProfileService.getBehaviorSummaryForSuggestion(1L)).thenReturn(aggressiveBehavior);
                when(positionRepository.findByUserUserIdOrderBySymbolAsc(1L))
                ...
                when(batchRepository.save(any(StockAiSuggestionBatch.class))).thenAnswer(invocation -> {
                        StockAiSuggestionBatch batch = invocation.getArgument(0);
                        batch.setSuggestionBatchId(22L);
                        return batch;
                });
                when(itemRepository.findBySuggestionBatchAndStatusInOrderByRankNoAsc(any(), anyCollection()))
                                .thenReturn(List.of());
                ...
                ArgumentCaptor<String> promptCaptor = ArgumentCaptor.forClass(String.class);
                verify(openAiClient, atLeastOnce()).generateSuggestion(anyString(), promptCaptor.capture());
                JsonNode root = new ObjectMapper().readTree(promptCaptor.getAllValues().get(0));
        ...
        @Test
        void promptTreatsBehaviorAsLowConfidenceWhenPortfolioResetIsNewerThanBehaviorProfile() throws Exception {
                PaperTradingAccount account = new PaperTradingAccount();
                account.setLastResetAt(LocalDateTime.of(2026, 6, 8, 13, 0));
        ...
        @Test
        void generateSuggestionsForUser_shiftsPersonalizationFromOnboardingToBehaviorAsBehaviorConfidenceIncreases()
                        throws Exception {
                UserInvestmentProfile aggressiveProfile = profile();
                aggressiveProfile.setRiskTolerance(RiskTolerance.AGGRESSIVE);
                aggressiveProfile.setInvestmentGoal(InvestmentGoal.GROWTH);
                ...
                assertTrue(signalFor(stageA, "TSLA").path("combinedFitScore").asInt() > signalFor(stageA, "KO")
                                .path("combinedFitScore").asInt());
                ...
                assertEquals("conservative",
                                stageB.path("observedPaperTradingBehavior").path("favoriteRiskCategory").asText());
                ...
                assertTrue(signalFor(stageB, "KO").path("behaviorFitScore").asInt() > signalFor(stageB, "TSLA")
                                .path("behaviorFitScore").asInt());
                assertTrue(signalFor(stageB, "KO").path("combinedFitScore").asInt() > signalFor(stageA, "KO")
                                .path("combinedFitScore").asInt());

                assertEquals("HIGH", stageC.path("observedPaperTradingBehavior").path("behaviorConfidence").asText());
                assertEquals("ACTIVE_TRADER",
                                stageC.path("observedPaperTradingBehavior").path("behaviorStyle").asText());
                assertEquals("KO,JNJ,MSFT",
                                stageC.path("observedPaperTradingBehavior").path("mostTradedSymbols").asText());
                assertEquals("LOW",
                                stageC.path("observedPaperTradingBehavior").path("highVolatilityExposure").asText());
                ...
                assertTrue(signalFor(stageC, "JNJ").path("combinedFitScore").asInt() > signalFor(stageC, "TSLA")
                                .path("combinedFitScore").asInt());
                assertTrue(signalFor(stageC, "JNJ").path("combinedFitScore").asInt() >= signalFor(stageB, "JNJ")
                                .path("combinedFitScore").asInt());

                assertEquals(3, savedBatches.size());
                assertEquals("stock-suggestion-v4", savedBatches.get(0).getPromptVersion());
        ...
        void generateSuggestionsForUser_retriesAllAggressiveOutputWhenHighConfidenceBehaviorIsLowerRisk() {
                ...
                when(batchRepository
                                ...
                                .thenReturn(Optional.empty());
                when(behaviorProfileService.getBehaviorSummaryForSuggestion(1L))
                                .thenReturn(highActiveLowerRiskBehavior);
                when(openAiClient.generateSuggestion(anyString(), anyString()))
                                .thenReturn(
                ...
                assertEquals(StockAiSuggestionBatchStatus.SUCCESS, savedBatches.get(0).getStatus());
                assertTrue(savedItems.stream().anyMatch(item -> item.getSymbol().equals("JNJ")));
                assertFalse(savedItems.stream()
                                .allMatch(item -> List.of("NVDA", "TSLA", "AMD").contains(item.getSymbol())));
                verify(behaviorProfileService).getBehaviorSummaryForSuggestion(1L);
                verify(behaviorProfileService, never()).createLowConfidenceProfileIfMissing(any());
        }
        ...
        void repeatedOpenAiFailureWithSameInputHashReusesExistingRuleBasedBatch() {
                ...
                assertTrue(fallbackItems.stream()
                                .allMatch(item -> item.getStatus() == StockAiSuggestionItemStatus.ACTIVE));
        ...
        void existingFallbackCachedSameInputHashIsUpdatedToRuleBasedAfterOpenAiFailure() {
                ...
                StockAiSuggestionItem existingFallbackItem = suggestionItem(existingFallbackCached,
                                StockAiSuggestionItemStatus.ACTIVE);
                ...
                when(openAiClient.generateSuggestion(anyString(), anyString()))
                                .thenReturn(OpenAiSuggestionResult.failure("OpenAI unavailable"));
                when(itemRepository.findByUserUserIdAndStatus(1L, StockAiSuggestionItemStatus.ACTIVE))
                                .thenReturn(List.of(existingFallbackItem));
                when(itemRepository.findBySuggestionBatchOrderByRankNoAsc(existingFallbackCached))
                                .thenReturn(List.of(existingFallbackItem));
                when(batchRepository.save(any(StockAiSuggestionBatch.class))).thenAnswer(invocation -> {
                        StockAiSuggestionBatch batch = invocation.getArgument(0);
        ...
        void existingRuleBasedSameInputHashIsReusedBeforeOpenAiCall() {
                UserInvestmentProfile profile = profile();
                StockAiSuggestionBatch existingRuleBased = batch(profile,
                                StockAiSuggestionBatchStatus.FALLBACK_RULE_BASED);
                existingRuleBased.setSuggestionBatchId(72L);
                StockAiSuggestionItem existingRuleBasedItem = suggestionItem(existingRuleBased,
                                StockAiSuggestionItemStatus.ACTIVE);
        ...
        void openAiValidationFailureWithSameInputFallbackCachedUpdatesToCurrentRuleBasedFallback() {
                ...
                BehaviorSummaryForSuggestion highConservativeBehavior = behaviorSummary(
                                ...
                                36,
                                22);
                StockAiSuggestionBatch staleFallbackCached = batch(aggressiveProfile,
                                StockAiSuggestionBatchStatus.FALLBACK_CACHED);
                ...
                StockAiSuggestionItem staleNvda = suggestionItem(staleFallbackCached,
                                StockAiSuggestionItemStatus.ACTIVE);
                staleNvda.setSymbol("NVDA");
                staleNvda.setRankNo(1);
                staleNvda.setRiskLevel("aggressive");
                staleNvda.setAnalysisSnapshot(adaptiveSnapshot("NVDA"));
                StockAiSuggestionItem staleTsla = suggestionItem(staleFallbackCached,
                                StockAiSuggestionItemStatus.ACTIVE);
                ...
                StockAiSuggestionItem staleAmd = suggestionItem(staleFallbackCached,
                                StockAiSuggestionItemStatus.ACTIVE);
                ...
                List<StockAiSuggestionItem> activeSavedItems = savedItems.stream()
                                ...
                                .toList();
                assertEquals(List.of("KO", "JNJ", "MSFT"),
                                activeSavedItems.stream().map(StockAiSuggestionItem::getSymbol).toList());
                assertTrue(activeSavedItems.stream().map(StockAiSuggestionItem::getRiskLevel)
                                .noneMatch("aggressive"::equals));
                assertTrue(activeSavedItems.stream()
                                .allMatch(item -> item.getSuggestionBatch() == staleFallbackCached));
                assertEquals(StockAiSuggestionItemStatus.EXPIRED, staleNvda.getStatus());
                ...
                verify(batchRepository, never()).findTopByUserUserIdAndStatusAndExpiresAtAfterOrderByCreatedAtDesc(
                                ...
                                eq(StockAiSuggestionBatchStatus.SUCCESS),
                                any());
                verify(batchRepository, never()).findTopByUserUserIdAndStatusOrderByCreatedAtDesc(1L,
        ...
        private String inputHashForGeneratedFallback(
                ...
                lenient().when(batchRepository.findTopByUserUserIdAndStatusOrderByCreatedAtDesc(
                                eq(1L),
                                eq(StockAiSuggestionBatchStatus.SUCCESS))).thenReturn(Optional.empty());
                when(itemRepository.findByUserUserIdAndStatus(1L, StockAiSuggestionItemStatus.ACTIVE))
                                .thenReturn(List.of());
                when(batchRepository.save(any(StockAiSuggestionBatch.class))).thenAnswer(invocation -> {
                        StockAiSuggestionBatch batch = invocation.getArgument(0);
        ...
        private PaperPosition paperPosition(String symbol, int quantity, String totalCost) {
                PaperPosition position = new PaperPosition();
                position.setUser(user);
                position.setSymbol(symbol);
                position.setQuantity(quantity);
                position.setAverageCost(new BigDecimal(totalCost).divide(BigDecimal.valueOf(quantity), 4,
        ...
        private ArgumentCaptor<String> refreshWithGoogOpenAiResults(
                ...
                when(batchRepository
                                .findTopByUserUserIdAndModelAndPromptVersionAndInputHashAndStatusInOrderByCreatedAtDesc(
                                                ...
                                                anyCollection()))
                                .thenReturn(Optional.empty());
                when(openAiClient.generateSuggestion(anyString(), anyString())).thenReturn(firstResult,
                                additionalResults);
                lenient().when(batchRepository.findTopByUserUserIdAndStatusAndExpiresAtAfterOrderByCreatedAtDesc(
                                eq(1L),
                                eq(StockAiSuggestionBatchStatus.SUCCESS),
